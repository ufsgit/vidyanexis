...
import 'package:vidyanexis/controller/models/stock_use_report_model.dart';
...
class StockUseReportProvider extends ChangeNotifier {
  ...
  Future<void> searchReport(BuildContext context) async {
    try {
      ...
      final response = await HttpRequest.httpGetRequest(
        endPoint:
            '${HttpUrls.searchStockUseReport}?Customer_Name=${Uri.encodeComponent(_customerName)}&Item_Name=${Uri.encodeComponent(_itemName)}&Is_Date=$isDate&Fromdate=$_formattedFromDate&Todate=$_formattedToDate',
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data != null) {
          _reportList = (data as List<dynamic>)
              .map((item) => StockUseReportModel.fromJson(item))
              .toList();
        } else {
          _reportList = [];
        }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data != null && data is List) {
          ...
          // Remove metadata item if present (usually the last item)
