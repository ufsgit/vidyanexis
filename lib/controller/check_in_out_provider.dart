...
class CheckInOutProvider extends ChangeNotifier {
  ...
  void updateStatus(AttendanceDetails item, bool bool) {
  ...
  getAttendanceDetails(String editId, BuildContext context) async {
    try {
      ...
      final response = await HttpRequest.httpGetRequest(
          endPoint:
              '${HttpUrls.getAttendanceDetails}?Attendance_Master_Id=$editId');
      ...
      if (response.statusCode == 200) {
        ...
        if (data != null) {
          ...
          _attendanceDetails = (dataItem as List<dynamic>)
              .map((item) => AttendanceDetails.fromJson(item))
              .toList();
  ...
  void saveAttendance(
      BuildContext context, List<Map<String, Object>> editedList) async {
    print(editedList);
    try {
      Loader.showLoader(context);
