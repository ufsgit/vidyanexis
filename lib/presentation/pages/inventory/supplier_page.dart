...
class _SupplierPageState extends State<SupplierPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: ClipRRect(
          borderRadius: BorderRadius.circular(4),
          child: settingsProvider.searchSupplier.isEmpty
              ? _buildEmptyState()
              : Column(
                  children: settingsProvider.searchSupplier
                      .asMap()
                      .entries
                      .map((entry) {
                    final i = entry.key;
                        ...
                        _buildRow(
                          ...
                          onDelete: settingsProvider.menuIsDeleteMap[45] == 1
                              ? () => _showDeleteDialog(context,
                                  settingsProvider, supplier.supplierId)
                              : null,
                        ),
                        if (i < settingsProvider.searchSupplier.length - 1)
                          const Divider(
                              height: 1,
                              thickness: 1,
  ...
  Widget _buildEmptyState() {
    return const CommonEmptyState(message: 'No suppliers found');
  }
